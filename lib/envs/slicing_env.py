...
class SlicingEnvironment(gym.Env):
    ...
    def __init__(self, traffic_df, max_num_packets, max_size_per_tti, num_action_lvls, num_slices,
                 ...
                 c1_urllc, c1_video, c2_volte, c2_urllc, c2_video):
    ...
    def reset(self):
        ...
        if self.ep_num == self.total_data_episodes:
            self.ep_num = 0
            self.finish_time_volte = 0
            self.finish_time_urllc = 0
            self.finish_time_video = 0
    ...
    def _next_observation(self):
        ...
        self.obs = str([[round(volte_relative_demand,1)], [round(urllc_relative_demand,1)], 
                        [round(video_relative_demand,1)]])
...
# latency sigmoid-based reward function
        ...
        done = self.max_num_steps <= self.current_step
    ...
    def _take_action(self, action):
        ...
        current_video_traffic = self.traffic_df[(self.traffic_df['type']=='video') &
                   (self.traffic_df['arrival_times'] >= self.current_step*self.sl_win_size) &
                   (self.traffic_df['arrival_times'] < self.sl_win_size*(self.current_step+1)) &
                   (self.traffic_df['user'] <= self.num_users_poisson[0][self.current_step])]
        
        current_volte_traffic = self.traffic_df[(self.traffic_df['type']=='volte') &
                   (self.traffic_df['arrival_times'] >= self.current_step*self.sl_win_size) &
                   (self.traffic_df['arrival_times'] < self.sl_win_size*(self.current_step+1)) &
                   (self.traffic_df['user'] <= self.num_users_poisson[1][self.current_step])]

        
        current_urllc_traffic = self.traffic_df[(self.traffic_df['type']=='urllc') &
                   (self.traffic_df['arrival_times'] >= self.current_step*self.sl_win_size)
                   & (self.traffic_df['arrival_times'] < self.sl_win_size*(self.current_step+1))
                   & (self.traffic_df['user'] <= self.num_users_poisson[2][self.current_step])]
